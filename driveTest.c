...
#define IR_LIMIT 1910 // about 16 cm or 1.54V
...
//    }
    ...
    // CANYON NAV
    while (1) {
        if(getRangeValue(&frontRange) < IR_LIMIT) { // if we see a wall in front of us
            if(getRangeValue(&leftRange) < IR_LIMIT) { // there is a wall to the left
                // turn right
                setSpeed(0);
                turn(90, 15);
            } else {
...
void setup() {
    ...
    frontRange.readBit = &IRRANGE1;
    leftRange.readBit = &IRRANGE2;
    ...
    OC2CON1 = 0x1C06;
    OC2CON2 = 0x001F;
    OC2RS = 1249; // Fpwm = 200 --> 1 RPS
    OC2R = 0; // keep motor off for now
}
